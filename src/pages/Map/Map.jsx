		...
		GameContext.socket.onmessage = (msg) => {
			...
			switch (action.Type) {
				case 'updateTroops':
					...
					break;
				case 'updateCountry':
					let ok = typeof GameContext.countryStates[action.Country] === 'undefined';
					if (ok || getOwner(GameContext.countryStates[action.Country].Player) !== getOwner(action.Player)) {
						if (
							Object.keys(GameContext.capitals).some((key) => GameContext.capitals[key] == action.Country)
						) {
							//change allegiance
							GameContext.allegiances[GameContext.countryStates[action.Country].Player] = getOwner(
								action.Player
							);
						}
