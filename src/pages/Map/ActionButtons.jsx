  ...
  return !showAlliance ? (
    <div>
      <Button
        ...
        style={{ textAlign: "center", alignItems: "center" }}
      >
        PROPOSE ALLIANCE (BETA)
      </Button>
      {alliances.map((alliance) =>
        <ListItem >
          {alliance.isProposal ?
            <div>
              <Typography>Ally with {alliance.name} for {alliance.cost} stonks?</Typography>
              <Button
                color="primary"
                onClick={() => {
                  acceptAlliance(alliance.name, alliance.cost, user, socket);
                }}
              >YES</Button>
              <Button
                color="secondary"
                onClick={() => {
                  declineAlliance(alliance.name, user, socket);
                }}
              >NO</Button>
            </div>
            :
            <Typography>Allied with {alliance.name}</Typography>}
        </ListItem>)}
    </div >
  ) : (
    <table cellSpacing="5px" style={{ minWidth: "100%" }}>
      ...
      <tr>
        <Button
          ...
          className={classes.button}
          onClick={() => {
            proposeAlliance(targetPlayer, numTroops, user, socket);
            reset();
          }}
        >
          PROPOSE ALLIANCE
...
let acceptAlliance = takeAction((targetPlayer, cost, user, socket) => {
  return new action(
    cost,
    "allianceaccept",
    targetPlayer,
